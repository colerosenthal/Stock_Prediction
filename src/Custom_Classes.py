...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for w in self.windows:
            ...
            rolling_std = data.rolling(w, min_periods=w).std()
            X_out[f'BB_upper_{w}'] = X_out[f'MA_{w}'] + (2 * rolling_std)
            X_out[f'BB_lower_{w}'] = X_out[f'MA_{w}'] - (2 * rolling_std)

            # 7. MACD (Moving Average Convergence Divergence)
            ema_fast = data.ewm(span=w, min_periods=w).mean()
            ema_slow = data.ewm(span=w * 2, min_periods=w * 2).mean()
            X_out[f'MACD_{w}'] = ema_fast - ema_slow

            # 8. ATR (Average True Range - volatility indicator)
            high_low = data.rolling(w).max() - data.rolling(w).min()
            X_out[f'ATR_{w}'] = high_low.rolling(w, min_periods=w).mean()
